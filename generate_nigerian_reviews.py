...
Uses Gemini to synthesize structured Nigerian restaurant reviews.
...
# If a model hits rate limits or fails, we fall back to the next one.
MODEL_CHAIN = [
...
# Track which model we are currently using
current_model_index = 0


def get_current_model():
    return MODEL_CHAIN[current_model_index]
...
def advance_model():
    ...
    if current_model_index < len(MODEL_CHAIN) - 1:
        ...
        print(f"    Switching to fallback model: {get_current_model()}")
        return True
    return False
...
IMPORTANT: Return ONLY a raw JSON array. Do not wrap in markdown code fences. Do not add any text before or after the array. Start your response directly with [ and end with ].
...
def extract_json_array(text):
    ...
    # Remove all lines that are just backtick fences
    lines = text.split("\n")
...
def generate_batch(client, city, price_sensitivity, tone, restaurant_type, rating_range, count):
    ...
    while True:
        ...
        try:
            ...
            # Rate limit — wait and retry on same model
            if "429" in err and "RESOURCE_EXHAUSTED" in err:
                ...
                print(f"    Rate limit hit on {model}. Waiting {wait}s then retrying...")
                time.sleep(wait)
                # After waiting, try advancing model too
                if not advance_model():
                    print("    All models exhausted. Skipping batch.")
                    return []
                continue
            ...
            # Model not found — advance immediately
...
def main():
    ...
    print(f"Loaded {len(HANDCRAFTED)} handcrafted samples")
    print()
    ...
    for i, (city, price, tone, rtype, ratings, count) in enumerate(BATCH_CONFIGS):
        ...
        if batch:
            ...
            print(f"    OK - got {len(batch)} reviews, total: {len(all_reviews)}")
        else:
